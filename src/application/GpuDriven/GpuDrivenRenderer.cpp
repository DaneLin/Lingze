	    ...
	    lz::RenderGraph::ComputePassDesc()
	        ...
	        .set_record_func([&](lz::RenderGraph::PassContext context) {
		        ...
		        auto shader_data = frame_info.memory_pool->begin_set(shader_data_set_info);
		        {
			        ...
			        cull_data->draw_count  = uint32_t(scene->get_draw_count());
		        }

		        frame_info.memory_pool->end_set();

		        std::vector<lz::StorageBufferBinding> storage_buffer_bindings;
	        	storage_buffer_bindings.push_back(shader_data_set_info->make_storage_buffer_binding("MeshData", &scene->get_global_mesh_info_buffer()));
		        storage_buffer_bindings.push_back(shader_data_set_info->make_storage_buffer_binding("MeshDrawData", &scene->get_global_mesh_draw_buffer()));
		        ...
		        uint32_t dispatch_x = uint32_t((scene->get_draw_count() + 31) / 32);
...
{
	visible_mesh_draw_proxy_  = core->get_render_graph()->add_buffer<lz::MeshDrawCommand>(uint32_t(render_context->get_draw_count()));
