...
bool MeshShadingApp::load_scene()
{
	...
	bool        result           = load_scene_from_file(config_file_name,
	                                                    lz::Scene::GeometryTypes::eTriangles);
...
void MeshShadingApp::render_ui()
{
	...
	ImGui::Begin("Demo features", 0, ImGuiWindowFlags_NoScrollbar);
	{
		...
		{
			dynamic_cast<lz::render::MeshShadingRenderer *>(renderer_.get())
			    ->set_mesh_shading_enable_flag(mesh_shading_enabled_);
		}
	}
	ImGui::End();
}
