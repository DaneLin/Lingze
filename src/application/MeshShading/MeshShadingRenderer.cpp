...
namespace lz::render
{
		...
		const lz::Camera& camera, const lz::Camera& light, lz::Scene* scene, GLFWwindow* window)
	{
		...
		core_->get_render_graph()->add_pass(lz::RenderGraph::RenderPassDesc()
			...
			.set_record_func([&](lz::RenderGraph::RenderPassContext context)
				{
					auto shader_program = base_shape_shader_.shader_program_.get();
					...
					auto shader_data = frame_info.memory_pool->begin_set(shader_data_set_info);
					{
						...
						auto shader_data_buffer = frame_info.memory_pool->get_uniform_buffer_data<DataBuffer>("ubo_data");// same name from shader
					...
					context.get_command_buffer().drawIndexedIndirect(scene->get_draw_indirect_buffer().get_handle(), 0, 6, sizeof(vk::DrawIndexedIndirectCommand));
	...
	void MeshShadingRenderer::reload_shaders()
	{
		...
		{
			base_shape_shader_.vertex_shader_.reset(new Shader(logical_device, SHADER_SPIRV_DIR"MeshShading/BasicShape.vert.spv"));
			base_shape_shader_.fragment_shader_.reset(new Shader(logical_device, SHADER_SPIRV_DIR"MeshShading/BasicShape.frag.spv"));
			base_shape_shader_.shader_program_.reset(new ShaderProgram({ base_shape_shader_.vertex_shader_.get(), base_shape_shader_.fragment_shader_.get() }));
		}
		{
			meshlet_shader_.mesh_shader_.reset(new Shader(logical_device, SHADER_SPIRV_DIR "MeshShading/meshlet.mesh.spv"));
			meshlet_shader_.fragment_shader_.reset(new Shader(logical_device, SHADER_SPIRV_DIR "MeshShading/meshlet.frag.spv"));
			meshlet_shader_.shader_program_.reset(new ShaderProgram({ meshlet_shader_.mesh_shader_.get(), meshlet_shader_.fragment_shader_.get() }));
		}
	}
