...
void MaterialSystem::initialize()
{
	...
	texture_binding.setBinding(k_bindless_texture_binding)
	    .setDescriptorType(vk::DescriptorType::eCombinedImageSampler)
	    .setDescriptorCount(k_max_bindless_resources)
	...
	auto descriptor_sets     = core_->get_logical_device().allocateDescriptorSetsUnique(alloc_info);
	bindless_descriptor_set_ = descriptor_sets[0].get();
}
uint32_t MaterialSystem::register_material(const std::shared_ptr<Material> &material)
{
	...
	params.base_color_factor                = material->base_color_factor;
	params.metallic_factor                  = material->metallic_factor;
	params.roughness_factor                 = material->roughness_factor;
	params.emissive_factor                  = material->emissive_factor;
	params.diffuse_texture_index            = upload_texture(material->diffuse_texture);
	params.normal_texture_index             = upload_texture(material->normal_texture);
	params.metallic_roughness_texture_index = upload_texture(material->metallic_roughness_texture);
	params.emissive_texture_index           = upload_texture(material->emissive_texture);
	params.occlusion_texture_index          = upload_texture(material->occlusion_texture);

	// MaterialParameters *mapped_params = static_cast<MaterialParameters *>(material_parameters_buffer_->map());
	// mapped_params[material_index]     = params;
...
void MaterialSystem::process_pending_updates(vk::CommandBuffer cmd_buffer)
{
	...
	for (const auto &request : updates)
	{
		if (request.type == UpdateRequest::UpdateType::eTextureUpload)
		{
			...
			{
				texture_index = texture_name_to_index_[request.texture->name];
			}
			else
			{
				continue;
			...
			descriptor_writes.push_back(vk::WriteDescriptorSet()
			                                .setDstSet(bindless_descriptor_set_)
		...
		else if (request.type == UpdateRequest::UpdateType::eMaterialParametersUpdate)
		{
            ...
            params.diffuse_texture_index            = texture_name_to_index_[materials_[material_index]->diffuse_texture->name];
            params.normal_texture_index             = texture_name_to_index_[materials_[material_index]->normal_texture->name];
            params.metallic_roughness_texture_index = texture_name_to_index_[materials_[material_index]->metallic_roughness_texture->name];
            params.emissive_texture_index           = texture_name_to_index_[materials_[material_index]->emissive_texture->name];
            params.occlusion_texture_index          = texture_name_to_index_[materials_[material_index]->occlusion_texture->name];
        }
	}
...
vk::DescriptorSet MaterialSystem::get_bindless_descriptor_set() const
{
