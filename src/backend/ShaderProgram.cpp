...
ImageSamplerBinding::ImageSamplerBinding() :
    image_view(nullptr), sampler(nullptr)
{
}

ImageSamplerBinding::ImageSamplerBinding(lz::ImageView *image_view, lz::Sampler *sampler,
                                         uint32_t shader_binding_id) :
    image_view(image_view), sampler(sampler), shader_binding_id(shader_binding_id)
{
	assert(image_view);
	assert(sampler);
}
...
{
	return std::tie(image_view, sampler, shader_binding_id) < std::tie(other.image_view, other.sampler, other.shader_binding_id);
}

UniformBufferBinding::UniformBufferBinding() :
    buffer(nullptr), offset(-1), size(-1)
{
}
...
{
	return std::tie(buffer, shader_binding_id, offset, size) < std::tie(other.buffer, other.shader_binding_id, other.offset, other.size);
}
...
StorageImageBinding::StorageImageBinding() :
    image_view(nullptr)
{
}
...
{
	return std::tie(name, shader_binding_index, pod_part_size, array_member_size) <
	       std::tie(other.name, other.shader_binding_index, other.pod_part_size, other.array_member_size);
}
...
bool DescriptorSetLayoutKey::operator<(const DescriptorSetLayoutKey &other) const
{
	return std::tie(uniform_datum_, uniform_buffer_datum_, image_sampler_datum_, storage_buffer_datum_,
	                storage_image_datum_) < std::tie(other.uniform_datum_, other.uniform_buffer_datum_, other.image_sampler_datum_,
...
DescriptorSetLayoutKey DescriptorSetLayoutKey::merge(DescriptorSetLayoutKey *set_layouts, size_t sets_count)
{
	...
	for (auto &uniform_buffer_binding : uniform_buffer_bindings)
	{
		...
		for (size_t set_index = 0; set_index < sets_count; set_index++)
		{
			...
			if (!src_uniform_buffer_id.is_valid())
				continue;
	...
	for (auto &image_sampler_binding : image_sampler_bindings)
	{
		...
		for (size_t set_index = 0; set_index < sets_count; ++set_index)
		{
			...
			if (!src_image_sampler_id.is_valid())
				continue;
	...
	for (auto &storage_buffer_binding : storage_buffer_bindings)
	{
		...
		for (size_t set_index = 0; set_index < sets_count; ++set_index)
		{
			...
			if (!src_storage_buffer_id.is_valid())
				continue;
	...
	for (auto &storage_image_binding : storage_image_bindings)
	{
		...
		for (size_t set_index = 0; set_index < sets_count; set_index++)
		{
			...
			if (!src_storage_image_id.is_valid())
				continue;
