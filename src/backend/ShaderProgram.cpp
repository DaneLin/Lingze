...
namespace lz
{
	ImageSamplerBinding::ImageSamplerBinding(): imageView(nullptr), sampler(nullptr)
	{
	}

	ImageSamplerBinding::ImageSamplerBinding(lz::ImageView* imageView, lz::Sampler* sampler, uint32_t shaderBindingId): imageView(imageView), sampler(sampler), shaderBindingId(shaderBindingId)
	{
		assert(imageView);
		assert(sampler);
	}
	...
	UniformBufferBinding::UniformBufferBinding(lz::Buffer* buffer, uint32_t shaderBindingId, vk::DeviceSize offset,
		vk::DeviceSize size): buffer(buffer), shaderBindingId(shaderBindingId), offset(offset), size(size)
	{
		assert(buffer);
	}
	...
	StorageBufferBinding::StorageBufferBinding(lz::Buffer* _buffer, uint32_t _shaderBindingId, vk::DeviceSize _offset,
	...
	StorageImageBinding::StorageImageBinding(): imageView(nullptr)
	{
	}

	StorageImageBinding::StorageImageBinding(lz::ImageView* _imageView, uint32_t _shaderBindingId): imageView(_imageView),
	...
	size_t DescriptorSetLayoutKey::GetUniformBuffersCount() const
	{
		return uniformBufferDatum.size();
	}

	void DescriptorSetLayoutKey::GetUniformBufferIds(UniformBufferId* dstUniformBufferIds, size_t count,
		size_t offset) const
	{
		if (count == -1)
		{
	...
	DescriptorSetLayoutKey::UniformBufferId DescriptorSetLayoutKey::GetUniformBufferId(std::string bufferName) const
	{
	...
	DescriptorSetLayoutKey::UniformBufferId DescriptorSetLayoutKey::GetUniformBufferId(uint32_t bufferBindingId) const
	{
	...
	DescriptorSetLayoutKey::UniformBufferData DescriptorSetLayoutKey::GetUniformBufferInfo(
		UniformBufferId uniformBufferId) const
	{
		return uniformBufferDatum[uniformBufferId.id];
	}

	UniformBufferBinding DescriptorSetLayoutKey::MakeUniformBufferBinding(std::string bufferName, lz::Buffer* buffer,
	...
	size_t DescriptorSetLayoutKey::GetStorageBuffersCount() const
	{
		return storageBufferDatum.size();
	}

	void DescriptorSetLayoutKey::GetStorageBufferIds(StorageBufferId* dstStorageBufferIds, size_t count,
		size_t offset) const
	{
		if (count == -1)
	...
	DescriptorSetLayoutKey::StorageBufferId DescriptorSetLayoutKey::GetStorageBufferId(
		std::string storageBufferName) const
	{
	...
	DescriptorSetLayoutKey::StorageBufferId DescriptorSetLayoutKey::GetStorageBufferId(uint32_t bufferBindingId) const
	{
	...
	DescriptorSetLayoutKey::StorageBufferData DescriptorSetLayoutKey::GetStorageBufferInfo(
		StorageBufferId storageBufferId) const
	{
		return storageBufferDatum[storageBufferId.id];
	}

	StorageBufferBinding DescriptorSetLayoutKey::MakeStorageBufferBinding(std::string bufferName, lz::Buffer* _buffer,
		vk::DeviceSize _offset, vk::DeviceSize _size) const
	{
	...
	size_t DescriptorSetLayoutKey::GetUniformsCount() const
	{
		return uniformDatum.size();
	}

	void DescriptorSetLayoutKey::GetUniformIds(UniformId* dstUniformIds, size_t count, size_t offset) const
	{
		if (count == -1)
		{
	...
	DescriptorSetLayoutKey::UniformData DescriptorSetLayoutKey::GetUniformInfo(UniformId uniformId) const
	{
		return uniformDatum[uniformId.id];
	}

	DescriptorSetLayoutKey::UniformId DescriptorSetLayoutKey::GetUniformId(std::string name) const
	{
	...
	size_t DescriptorSetLayoutKey::GetImageSamplersCount() const
	{
		return imageSamplerDatum.size();
	}

	void DescriptorSetLayoutKey::GetImageSamplerIds(ImageSamplerId* dstImageSamplerIds, size_t count,
		size_t offset) const
	{
		if (count == -1)
	...
	DescriptorSetLayoutKey::ImageSamplerData DescriptorSetLayoutKey::GetImageSamplerInfo(
		ImageSamplerId imageSamplerId) const
	{
		return imageSamplerDatum[imageSamplerId.id];
	}

	DescriptorSetLayoutKey::ImageSamplerId DescriptorSetLayoutKey::GetImageSamplerId(std::string imageSamplerName) const
	{
	...
	DescriptorSetLayoutKey::ImageSamplerId DescriptorSetLayoutKey::GetImageSamplerId(uint32_t shaderBindingIndex) const
	{
	...
	ImageSamplerBinding DescriptorSetLayoutKey::MakeImageSamplerBinding(std::string imageSamplerName,
		lz::ImageView* _imageView, lz::Sampler* _sampler) const
	{
	...
	size_t DescriptorSetLayoutKey::GetStorageImagesCount() const
	{
		return storageImageDatum.size();
	}

	void DescriptorSetLayoutKey::GetStorageImageIds(StorageImageId* dstStorageImageIds, size_t count,
		size_t offset) const
	{
		if (count == -1)
	...
	DescriptorSetLayoutKey::StorageImageId DescriptorSetLayoutKey::GetStorageImageId(std::string storageImageName) const
	{
	...
	DescriptorSetLayoutKey::StorageImageId DescriptorSetLayoutKey::GetStorageImageId(uint32_t bufferBindingId) const
	{
	...
	DescriptorSetLayoutKey::StorageImageData DescriptorSetLayoutKey::GetStorageImageInfo(
		StorageImageId storageImageId) const
	{
		return storageImageDatum[storageImageId.id];
	}

	StorageImageBinding DescriptorSetLayoutKey::MakeStorageImageBinding(std::string imageName,
		lz::ImageView* _imageView) const
	{
	...
	uint32_t DescriptorSetLayoutKey::GetTotalConstantBufferSize() const
	{
		return size;
	}

	bool DescriptorSetLayoutKey::IsEmpty() const
	{
	...
	DescriptorSetLayoutKey DescriptorSetLayoutKey::Merge(DescriptorSetLayoutKey* setLayouts, size_t setsCount)
	{
	...
	void DescriptorSetLayoutKey::RebuildIndex()
	{
	...
	Shader::Shader(vk::Device logicalDevice, std::string shaderFile)
	{
	...
	const std::vector<uint32_t> Shader::GetBytecode(std::string filename)
	{
	...
	lz::ShaderModule* Shader::GetModule()
	{
		return shaderModule.get();
	}

	size_t Shader::GetSetsCount()
	{
		return descriptorSetLayoutKeys.size();
	}

	const DescriptorSetLayoutKey* Shader::GetSetInfo(size_t setIndex)
	{
		return &descriptorSetLayoutKeys[setIndex];
	}

	glm::uvec3 Shader::GetLocalSize()
	{
		return localSize;
	}

	void Shader::Init(vk::Device logicalDevice, const std::vector<uint32_t>& bytecode)
	{
	...
	ShaderProgram::ShaderProgram(Shader* _vertexShader, Shader* _fragmentShader): vertexShader(_vertexShader), fragmentShader(_fragmentShader)
	{
	...
	size_t ShaderProgram::GetSetsCount()
	{
		return combinedDescriptorSetLayoutKeys.size();
	}

	const DescriptorSetLayoutKey* ShaderProgram::GetSetInfo(size_t setIndex)
	{
