    ...
    struct InFlightQueue
    {
        InFlightQueue(lz::Core* core, lz::WindowDesc windowDesc, uint32_t inFlightCount, vk::PresentModeKHR preferredMode)
        {
            ...
            for (size_t frameIndex = 0; frameIndex < inFlightCount; frameIndex++)
            {
                ...
                frame.shaderMemoryBuffer = std::unique_ptr<lz::Buffer>(new lz::Buffer(core->GetPhysicalDevice(), core->GetLogicalDevice(), 100000000, vk::BufferUsageFlagBits::eUniformBuffer, vk::MemoryPropertyFlagBits::eHostCoherent));
                frame.gpuProfiler = std::unique_ptr<lz::GpuProfiler>(new lz::GpuProfiler(core->GetPhysicalDevice(), core->GetLogicalDevice(), 512));
