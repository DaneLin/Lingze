...
namespace lz
{
	DescriptorSetBindings& DescriptorSetBindings::SetUniformBufferBindings(
	...
	DescriptorSetBindings& DescriptorSetBindings::SetImageSamplerBindings(
	...
	DescriptorSetBindings& DescriptorSetBindings::SetStorageBufferBindings(
	...
	DescriptorSetBindings& DescriptorSetBindings::SetStorageImageBindings(
	...
	DescriptorSetCache::DescriptorSetCache(vk::Device logicalDevice) : logicalDevice(logicalDevice)
	{
	...
	vk::DescriptorSetLayout DescriptorSetCache::GetDescriptorSetLayout(
		const lz::DescriptorSetLayoutKey& descriptorSetLayoutKey)
	{
		...
		if (!descriptorSetLayout)
		{
			...
			std::vector<lz::DescriptorSetLayoutKey::StorageBufferId> storageBufferIds;
			...
			std::vector<lz::DescriptorSetLayoutKey::ImageSamplerId> imageSamplerIds;
			...
			std::vector<lz::DescriptorSetLayoutKey::StorageImageId> storageImageIds;
	...
	vk::DescriptorSet DescriptorSetCache::GetDescriptorSet(lz::DescriptorSetLayoutKey setLayoutKey,
	...
	{
		auto setBindings = lz::DescriptorSetBindings()
	...
	vk::DescriptorSet DescriptorSetCache::GetDescriptorSet(lz::DescriptorSetLayoutKey setLayoutKey,
	                                                       const lz::DescriptorSetBindings& setBindings)
	{
		...
		if (!descriptorSet)
		{
			...
			std::vector<vk::DescriptorBufferInfo> uniformBufferInfos(setBindings.uniformBufferBindings.size());
			//cannot be kept in local variable
			for (size_t uniformBufferIndex = 0; uniformBufferIndex < setBindings.uniformBufferBindings.size();
			     uniformBufferIndex++)
			{
				...
				uniformBufferInfos[uniformBufferIndex] = vk::DescriptorBufferInfo()
				                                         .setBuffer(uniformBinding.buffer->get_handle())
			...
			for (size_t imageSamplerIndex = 0; imageSamplerIndex < setBindings.imageSamplerBindings.size();
			     imageSamplerIndex++)
			{
			...
			for (size_t storageBufferIndex = 0; storageBufferIndex < setBindings.storageBufferBindings.size();
			     storageBufferIndex++)
			{
				...
				storageBufferInfos[storageBufferIndex] = vk::DescriptorBufferInfo()
				                                         .setBuffer(storageBinding.buffer->get_handle())
			...
			for (size_t storageImageIndex = 0; storageImageIndex < setBindings.storageImageBindings.size();
			     storageImageIndex++)
			{
				...
				if (setBindings.imageSamplerBindings.size() > 0)
					storageImageInfos[storageImageIndex].setSampler(
						setBindings.imageSamplerBindings[0].sampler->GetHandle());
	...
	{
		return
			std::tie(layout, bindings.uniformBufferBindings, bindings.storageBufferBindings,
			         bindings.storageImageBindings, bindings.imageSamplerBindings) <
			std::tie(other.layout, other.bindings.uniformBufferBindings, other.bindings.storageBufferBindings,
			         other.bindings.storageImageBindings, other.bindings.imageSamplerBindings);
	}
}
