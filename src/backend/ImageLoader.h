...
namespace lz
{
			...
			for (auto& layer : mip.layers)
			{
				...
				for (size_t z = 0; z < size.z; z++)
				{
					for (size_t y = 0; y < size.y; y++)
					{
						...
						{
							uint8_t* texelPtr = texelData.texels.data() + currOffset + (x + y * mip.size.x + z * mip.
								size.x * mip.size.y) * texelData.texelSize;
	...
	static gli::texture LoadTexelDataToGli(ImageTexelData texelData)
	{
		...
		gli::texture texture(gli::target::TARGET_2D, format,
		                     gli::extent3d(texelData.baseSize.x, texelData.baseSize.y, texelData.baseSize.z),
		                     texelData.layersCount, 1, texelData.mips.size());
	...
	{
		auto gliData = lz::LoadTexelDataToGli(data);
	...
	static lz::ImageTexelData CreateSimpleImageTexelData(glm::uint8* pixels, int width, int height,
	                                                     vk::Format format = vk::Format::eR8G8B8A8Unorm)
	{
		lz::ImageTexelData texelData;
	...
	static void AddTransitionBarrier(lz::ImageData* imageData, lz::ImageUsageTypes srcUsageType,
	                                 lz::ImageUsageTypes dstUsageType, vk::CommandBuffer commandBuffer)
	{
		...
		commandBuffer.pipelineBarrier(srcImageAccessPattern.stage, dstImageAccessPattern.stage, vk::DependencyFlags(),
		                              {}, {}, {imageBarrier});
	}

	static void LoadTexelData(lz::Core* core, const ImageTexelData* texelData, lz::ImageData* dstImageData,
	                          lz::ImageUsageTypes dstUsageType = lz::ImageUsageTypes::GraphicsShaderRead)
	{
		auto stagingBuffer = std::unique_ptr<lz::Buffer>(new lz::Buffer(core->GetPhysicalDevice(),
		                                                                core->GetLogicalDevice(),
		                                                                texelData->texels.size(),
