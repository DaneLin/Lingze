                                 ...
                                 size_t frame_index_offset) const
{
	...
	{
		const size_t frame_index = (curr_frame_index_ - frame_index_offset - 1 - frame_number + 2 * frames_.size()) % frames_.size();
                                  ...
                                  size_t frame_index_offset)
{
	...
	for (size_t task_index = 0; task_index < curr_frame.tasks.size(); task_index++)
	{
		...
		glm::vec2 marker_right_rect_min = legend_pos + glm::vec2(
		                                                   ...
		                                                   legend_size.y - marker_rigth_rect_margin - (marker_right_rect_height + marker_right_rect_spacing) * stat.on_screen_index);
...
void ProfilersWindow::render()
{
	...
	title.precision(2);
	title << std::fixed << "Lingze profiler [" << 1.0f / avg_frame_time << "fps\t" << avg_frame_time * 1000.0f << "ms]###ProfilerWindow";
	// ###AnimatedTitle
